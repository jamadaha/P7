...
def main():
    ...
    parser.add_function(find_all_matches("steps:all", r"Plan length: (.+) step\(s\).\n", type=float), file=downward_log)
    parser.add_function(reduce_to_min("cost:all", "cost"), file=downward_log)
    parser.add_function(reduce_to_min("steps:all", "steps"), file=downward_log)
    parser.add_function(coverage, file=downward_log)
    parser.add_function(add_memory, file=downward_log)
    parser.parse()
