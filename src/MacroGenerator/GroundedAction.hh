...
struct GroundedLiteral {
    ...
    GroundedLiteral(unsigned int predicate, std::vector<std::vector<unsigned int>> objects) : predicate(predicate), objects(objects) {};
...
template<> struct std::hash<GroundedLiteral> {
    std::size_t operator()(GroundedLiteral const& s) const noexcept {
        ...
        for(auto& i : s.objects)
            h2 += std::hash<std::vector<unsigned int>>{}(i);
...
struct GroundedAction {
    ...
    const std::unordered_set<std::unordered_set<unsigned int>> parameters;
    ...
    std::unordered_set<std::unordered_set<unsigned int>> parameters,
